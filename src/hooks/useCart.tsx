...
export function CartProvider({ children }: CartProviderProps): JSX.Element {
  ...
  const addProduct = async (productId: number) => {
    try { //Verificar se o produto existe no carrinho para incrementar a quantidade, e verificar a quantidade no estoque
      ...
      const productExists = updatedCart.find(product => product.id === productId) // verificando se o produto existe no cart
  ...
  const removeProduct = (productId: number) => {
    try {
      ...
      const findIndex = updatedCart.findIndex(product => (product.id === productId))
      if(findIndex >= 0){
